...
void my_handler(int sig) {
    switch (sig) {
        case SIGTERM:
            running = false;
            cerr << "\nSEGTERM caught; finishing...\n";
            break;
        case SIGINT:
            running = false;
            cerr << "\nSIGINT caught; finishing...\n";
            break;
    }
}
...
void recget(int level, const vector<string> & urls, const string & parent = "") {
    if (level < 0)
        return;
    http_server serv;
    vector<unique_ptr<downloadable>> ds;
    for (auto i : urls) {
        if (!running) {
            cout << "Kinda stopped\n";
            return;
        ...
        if (i.substr(0, 7) == "http://") {
            if (dloaded.find(i.substr(0, 7)) != dloaded.end())
                continue; // we have already downloaded the file
            cerr << "Gonna dl " << i.substr(7) << '\n';
            ds.push_back(make_unique<downloadable>(serv, i.substr(7), "Referer: " + i));
            dloaded.emplace(i.substr(7));
        } else if(i.substr(0, 2) == "//") {
            if (dloaded.find(i.substr(0, 2)) != dloaded.end())
                continue; // we have already downloaded the file
            cerr << "Gonna dl " << i.substr(2) << '\n';
            ds.push_back(make_unique<downloadable>(serv, i.substr(2).c_str(), "Referer: " + i));
            ...
            ds.push_back(make_unique<downloadable>(serv, s, "Referer: " + i));
            dloaded.emplace(s);
        }
    }
